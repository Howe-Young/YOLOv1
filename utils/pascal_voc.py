...
class pascal_voc(object):
    def __init__(self, phase, rebuild=False):
        ...
        self.data_path = os.path.join(self.devkil_path, 'VOC2012')      #data/pascal_voc/VOCdevkit/VOC2012
            ...
            for idx in range(len(gt_labels_cp)):  #循环一次处理一张图片
                ...
                for i in range(self.cell_size):
                    for j in range(self.cell_size):
                        if gt_labels_cp[idx]['label'][i, j, 0] == 1: # 由于flip使用的水平翻转，因此对标签也做翻转（水平翻转y不变，只变换x）
    ...
    def load_labels(self):    #加载标签文件，获取标签
        ...
        Args:
        return:
            gt_labels: 每张图片的名字与标签相关信息的列表；
                    type: list(dict())
                    e.g.: [{'imname': '2008_003454.jpg', 'label': '(7, 7, 25)的numpy数组', 'flipped': False}]
        """
    ...
    def load_pascal_annotation(self, index):    # Load image and bounding boxes info from XML file in the PASCAL VOC format
        """
        Args:
            index: 图片名字，用于从本地加载图片和标注信息，比如：2008_003386
        return:
            label: 表示每张图片的标签，shape为(cell_size, cell_size, num_classes + 5), 这里每张图片划分为7*7个cell，pascal VOC一共有20类，
                所以shape为(7, 7, 25)；
                最后一维的顺序为[当前格子是否包含object(0 or 1), x_center, y_center, width, height, class_1, ..., class_20]
                这里x_center, y_center, width, height都是缩放后的坐标，比如将原图(1024, 768)的resize为(448, 448)，相应的box的坐标也需要做相应的缩放。
            len(objs): 每张图片包含的object数量
        """
        ...
        # shape (7, 7, 25), The 25-dimensional distribution is as follows:
        # [指示当前cell是否有object，x_center, y_center, width, height, class_1, ..., class_20]
        ...
        for obj in objs:     #记录出xml文件中object框的位置
            ...
            x_ind = int(boxes[0] * self.cell_size / self.image_size)   # 查看object的x_center落在哪个cell, 整张图片cell的数量为7*7，计算公式为： x_idx = x / (image_size / cell_size)
            y_ind = int(boxes[1] * self.cell_size / self.image_size)   # 查看object的y_center落在哪个cell， 整张图片cell的数量为7*7
            if label[y_ind, x_ind, 0] == 1:   #设置一个标记，看其是否被访问过,同时也表明这是个object。这里设置y在前，x在后是因为按照h*w的方式排列的
                continue
